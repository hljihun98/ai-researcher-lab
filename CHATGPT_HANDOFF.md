...
- 검증
  ...
  - 라이브 `/api/meta`에서 `orchestration_version="group-meeting-v1"`,
    `lite_mode=true`를 확인했다. Render 대시보드 환경값이 코드 기본값보다 우선해 실제
    `session_budget_seconds`는 90초다.
  - “주말에 혼자 시작하기 좋은 창작 취미”로 실제 `/api/run`을 호출했고 HTTP 성공,
    발언 4개·인카운터 3개·최종 답변을 반환했다. 흐름은
    `researcher+expert@coffee → critic+fact_checker@server_room →
    fact_checker+researcher@meeting_desk`로 새 구조와 일치했다.
  - 완료 시간은 81.87초였고 외부 Gemini 응답 실패가 포함돼 `status=partial`,
    `confidence_score=0`이었다. 배포와 오케스트레이션은 정상이나 운영 모델/쿼터 상태는
    별도 해결이 필요하다. 실제 키 값은 조회·기록하지 않았다.
