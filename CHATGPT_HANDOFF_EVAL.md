...
## GPT 평가 하네스 작업 완료 보고 → Claude (2026-07-22)

- 새 `eval/` 패키지에 `questions.json`, `harness.py`, `README.md`를 추가했다.
  - 7개 카테고리의 질문 14개를 포함한다.
  - 단일 모델 1회 답변과 `server.run_session_web()`의 멀티에이전트 최종 답변을 비교한다.
  - 질문 인덱스 홀짝으로 A/B 순서를 결정적으로 교대하고 심판 점수를 실제 답변 종류로
    다시 매핑한다.
  - 관련성·사실성·실행가능성·완결성 각 1~5점, 승자, 한 줄 근거를 엄격히 검증한다.
  - 심판 JSON이 코드펜스를 포함해도 파싱하며, 형식 오류는 중립 점수의 무효 판정으로
    보존하고 승률·평균 차이에서는 제외한다.
  - 질문 수(`--n`), 단계 간 지연(`--delay`, 기본 3초), 모델(`--model`), 출력 경로를
    CLI에서 설정할 수 있다. 결과는 콘솔 표와 `eval/results.json`에 저장한다.
- `eval/README.md`에 실제 사용법, 콘솔 예시, 무료 등급 호출량 및 유료 등급 권장 주의를
  기록했다. 생성 결과 파일은 `.gitignore`에 추가했다.
- `tests/test_eval_harness.py`에서 결정적 블라인드 순서, 역매핑, 데모 모드 1문항 완주와
  결과 JSON 필수 구조를 네트워크 없이 검증한다.
- 검증 결과
  - 데모 CLI `python -m eval.harness --n 1 --delay 0` 정상 완주.
  - 전체 단위 테스트 **39개 통과**.
  - `compileall` 통과, Ruff 오류 0건.
- 실제 Gemini 품질 평가는 키·쿼터를 소비하지 않도록 실행하지 않았다. 질문 하나가 단일
  답변·멀티에이전트 여러 호출·심판 호출을 포함하므로 무료 등급은 작은 `--n`으로 시작할 것.
- `server.py`와 `INDEX_HTML`/CSS/JS 및 기존 API 스키마는 수정하지 않았다.
